      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {item.watched && (
              <Chip size="sm" variant="flat" color="success">Watched</Chip>
            )}
            {item.isWinner && (
              <Chip size="sm" variant="flat" color="warning">Winner</Chip>
            )}
          </div>
        </div>
...
const WatchlistDetail = () => {
  ...
  const { isOpen: isMovieDetailOpen, onOpen: onMovieDetailOpen, onClose: onMovieDetailClose } = useDisclosure();
  ...
  const fetchWatchlistDetails = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsEditable(isAuthenticated && data.owner.id === token ? data.owner.id : false);
  ...
  const handleDragEnd = async (event) => {
    ...
    if (token) {
      setReordering(true);
      try {
        const response = await fetch(`http://localhost:8000/api/watchlists/categories/${watchlist.id}/reorder`, {
          method: 'POST',
          headers: {
  ...
  const confirmDeleteMovie = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/watchlists/categories/${watchlist.id}/movies/${movieToDelete}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleToggleLike = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/watchlists/categories/${watchlist.id}/like`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleShareWatchlist = () => {
    ...
    if (navigator.share) {
      navigator.share({
        ...
        text: watchlist.description || `Check out this watchlist: ${watchlist.name}`,
        url: shareUrl
      }).catch(error => {
  ...
  const filteredMovies = watchlist?.items.filter(item => {
  ...
  if (!watchlist) {
    return (
      <div className="text-center py-12">
        ...
        <Button as={Link} href="/watchlists" variant="flat" color="primary" className="mt-4">
          Go to My Watchlists
        </Button>
      </div>
    );
    ...
    <div className="space-y-6">
            ...
            <Dropdown>
              ...
              <DropdownMenu aria-label="Watchlist actions">
                <DropdownItem 
                  ...
                  onPress={() => router.push(`/watchlists/edit/${watchlist.slug}`)}
                >
      ...
      {watchlist.items.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-xl font-medium mb-2">No movies in this watchlist yet</p>
          {isEditable && (
            <Button
              color="primary"
              onPress={() => router.push(`/discover?addToWatchlist=${watchlist.id}`)}
      ...
      {/* Movie Detail Modal */}
      <Modal isOpen={isMovieDetailOpen} onClose={onMovieDetailClose} size="lg">
        <ModalContent>
          {selectedMovie && (
            <>
                  ...
                  <div className="md:w-2/3">
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {selectedMovie.watched && (
                            <Chip color="success" variant="flat">Watched</Chip>
                          )}
                          {selectedMovie.isWinner && (
                            <Chip color="warning" variant="flat">Movie Monday Winner</Chip>
                          )}
                        </div>
                      </div>
                      ...
                      {selectedMovie.userNote && (
                        <div>
                          <h4 className="font-medium text-sm text-default-500 mb-1">Your Notes</h4>
                      ...
                      {selectedMovie.userRating && (
                        <div>
                          <h4 className="font-medium text-sm text-default-500 mb-1">Your Rating</h4>
                          <div className="flex items-center">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                      ...
                      <div>
                        <h4 className="font-medium text-sm text-default-500 mb-1">Added to Watchlist</h4>
                        <p>{new Date(selectedMovie.addedAt).toLocaleDateString()}</p>
                      </div>
                      
                      {selectedMovie.watched && selectedMovie.watchedDate && (
                        <div>
                          <h4 className="font-medium text-sm text-default-500 mb-1">Watched Date</h4>
                          <p>{new Date(selectedMovie.watchedDate).toLocaleDateString()}</p>
                        </div>
                      )}
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)}>
        <ModalContent>
          ...
          <ModalBody>
            <p>Are you sure you want to remove this movie from the watchlist?</p>
          </ModalBody>
          <ModalFooter>
            <Button
              variant="flat"
