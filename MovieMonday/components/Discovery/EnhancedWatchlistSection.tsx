...
const EnhancedWatchlistSection: React.FC<EnhancedWatchlistSectionProps> = ({ 
  ...
  const sortedWatchlists = [...watchlists].sort((a, b) => b.likesCount - a.likesCount);
            ...
            <div className="relative h-full">
              ...
              <div className="absolute inset-0 p-4 grid grid-cols-2 gap-2 opacity-30">
                {watchlist.items?.slice(0, 4).map((movie, idx) => (
                  <div key={idx} className="aspect-[2/3] overflow-hidden rounded-md">
                    <Image
                      src={movie.posterPath ? `https://image.tmdb.org/t/p/w200${movie.posterPath}` : '/placeholder-poster.jpg'}
                      alt=""
              ...
              <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-background to-transparent z-[1]"></div>
              
              {/* Bottom content only */}
              <CardBody className="h-full flex flex-col justify-end relative z-[2] py-6">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="text-xl font-bold line-clamp-2 mr-3">{watchlist.name}</h3>
                  <div className="flex items-center bg-background/40 backdrop-blur-sm rounded-full px-3 py-1 ml-2 flex-shrink-0">
                    ...
                    <span className="font-semibold">{watchlist.likesCount}</span>
                  </div>
                </div>
              ...
              <CardBody className="p-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="w-14 h-20 flex-shrink-0 rounded overflow-hidden">
                    <Image
                      src={watchlist.items?.[0]?.posterPath 
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between mb-2">
                      <h3 className="font-bold truncate max-w-[160px]">{watchlist.name}</h3>
                    ...
                    <div className="flex gap-2">
                      <Chip size="sm" variant="flat" className="text-xs h-6">
                        <div className="flex items-center">
                          <Film className="h-3 w-3 mr-1" />
                          <span>{watchlist.moviesCount} movies</span>
                        </div>
